...
{
public:
	virtual INT64 Addr2Int64(SOCKADDR* pAddr,UDP_LONG streamId) = 0;//将地址与一个WORD转化为一个64位常量
...
public:
	...
	virtual IUdxTcp* Connect(char* ip,UDP_SHORT port,BOOL bSync = FALSE,INT64 dwUser = 0,INT64 expectbew = 0,int lostrat = 50,
		char*strCName = NULL,IMultCardTcp* pMultCard = NULL,UdxConnectInfo*pInfo = NULL) = 0;//通道名不得等于或超过25个字符
